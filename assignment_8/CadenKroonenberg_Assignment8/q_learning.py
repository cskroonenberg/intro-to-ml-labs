...
def checkConvergence(copyQ, Q):
    ...
    for i in range(1,numStates-1):
        for j in range(0,numStates):
            if R[i][j] != -1 and Q[i][j] == 0: # if state j is a potential next state from state i and no Q(i,j) value is recorded ...
                return False # ... Return false, all actions should be evaluated at least once
...
print("Episode 0 Q-Learning Value Matrix (V) (Initial Values)")
print_arr(Q)
...
for episode in range(1,maxEpisodes):
    ...
    while not check(findPosition(state)): # While not at a terminal state ...
        ...
        action = int(random.choice(actions)) # Randomly select next state
    ...
    if checkConvergence(copyQ, Q):
        print("\nEpisode {} Q-Learning Value Matrix (V) (Final Episode - CONVERGENCE DETECTED)".format(episode))
        print_arr(Q)
        exit()

    # Print on episode 1 and 10
    if episode in [1,10]:
        print("\nEpisode {} Q-Learning Value Matrix (V)".format(episode))
        print_arr(Q)
