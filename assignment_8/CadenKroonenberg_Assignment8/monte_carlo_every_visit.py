...
for epoch in range(1,maxEpochs):
    ...
    if comparison.all():
        print("\nEpisode {} (Final Episode - Every Visit Method - CONVERGENCE DETECTED)".format(epoch))
        printVals()
    ...
    if epoch in [1,10]:
        print("\nEpisode {} (Every Visit Method)".format(epoch))
        printVals()
        print("k, s, r, γ, G(s) values:")
        print(G_s_table)
